module "backend" {
  ...
  subnet_id = local.private_subnet_id # subnet_id = local.private_subnet_id
  ami = data.aws_ami.ami_info.id # doubt: why we need to choose openvpn ami
  tags = merge(
    var.common_tags,
    {
        Name = "${var.project_name}-${var.environment}-${var.common_tags.Component}"
...
resource "null_resource" "backend" {
    triggers = {
        instance_id = module.backend.id # this will be triggered everytime instance created
    }
    ...
    provisioner "remote-exec" {
        inline = [
            "chmod +x /tmp/${var.common_tags.Component}.sh", # to give execute accecss
...
resource "aws_ec2_instance_state" "backend" {
  instance_id = module.backend.id #doubt
  state       = "stopped"
...
resource "null_resource" "backend_delete" {
    ...
    provisioner "local-exec" {
        command = "aws ec2 terminate-instances --instance-ids i- ${module.backend.id}" # doubt
    }   
